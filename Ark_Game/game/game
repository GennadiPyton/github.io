...
let game = {
	...
	preload(callback) {
		...
		this.preloadSprites(onResourceLoag);
		this.preloadSprites(onResourceLoag);
	},
	preloadSprites(onResourceLoag) {
		for (let key in this.sprites) {
			this.sprites[key] = new Image();
			this.sprites[key].src = "img/" + key + ".png";
			this.sprites[key].addEventListener("load", onResourceLoag);
		}
	},
	preloadSprites(onResourceLoag) {
		for (let key in this.sounds) {
			this.sounds[key] = new Audio("sounds/" + key + ".mp3");
			this.sounds[key].addEventListener("canplaythrough", onResourceLoag, {once: true});
		}
	},
	create() {
	...
	update() {
		...
		this.ball.collideWorldBouds();
		this.platform.collideWorldBouds();
		this.platform.move();
		this.ball.move();
	},
	addScore() {
		...
		if (this.score >= this.blocks) {
			this.end("You win");
		}
	},
	collideBlocks() {
		for (let block of this.blocks) {
			if (blocks.active && this.ball.collide(block)) {
